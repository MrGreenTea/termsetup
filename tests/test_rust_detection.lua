-- Test the Rust project detection from workspaces.lua
...
-- Helper function to extract Rust detection results
local function detect_rust_details(path)
	local result = detect_project_details(path)
	-- Only return result if it's a Rust project
	if result and result.project_type == "rust" then
		return result
	end
	return nil
end
...
local test_cases = {
	{ "test-rust-projects/workspace-project", "workspace", "cargo", 2, true }, -- Cargo.toml with [workspace]
	{ "test-rust-projects/binary-project", "binary", "cargo", 2, true }, -- Cargo.toml with [[bin]]
	{ "test-rust-projects/library-project", "library", "cargo", 1, true }, -- Cargo.toml with [lib]
	{ "test-rust-projects/basic-project", "binary", "cargo", 1, true }, -- Cargo.toml only
	{ "test-rust-projects/mixed-project", "workspace", "cargo", 2, true }, -- Should prioritize workspace
	{ "test-rust-projects", nil, nil, 0, false }, -- Should not detect
}

print("Testing Rust project detection:")
local total_tests = 0
...
for i, test in ipairs(test_cases) do
	...
	local result = detect_rust_details(path)
	...
	elseif should_detect and result then
		...
		if result.project_subtype ~= expected_subtype then
			...
			table.insert(
				details,
				string.format(
					"Subtype: got '%s', expected '%s'",
					tostring(result.project_subtype),
					tostring(expected_subtype)
				)
			)
		end
		...
		if result.package_manager ~= expected_pkg_mgr then
			...
			table.insert(
				details,
				string.format(
					"Package mgr: got '%s', expected '%s'",
					tostring(result.package_manager),
					tostring(expected_pkg_mgr)
				)
			)
		end
		...
		if #result.detection_evidence ~= expected_evidence_count then
			...
			table.insert(
				details,
				string.format(
					"Evidence count: got %d, expected %d",
	...
	if test_passed then
		...
		if result then
			print(
				string.format(
					"    Got: subtype='%s', pkg_mgr='%s', evidence=%d",
					tostring(result.project_subtype),
					tostring(result.package_manager),
					#result.detection_evidence
				)
