...
class TestExtractCommitDescription:
    ...
    @pytest.mark.parametrize("diff_content,commit_type,expected", [
        # Function detection
...
class TestGenerateCommitMessage:
    ...
    @pytest.mark.parametrize("commit_type,staged_files,diff_content,expected", [
        # Single directory scope
        ("feat", ["src/utils.py"], "+def new_func():", "feat(src): add new_func function"),
        ("fix", ["tests/test_utils.py"], "+def test_fix():", "fix(tests): fix test_fix function"),  # Fix uses "fix" not "add"
        
        # No scope (root files)
        ("chore", ["config.json"], None, "chore: "),  # No description when diff_content is None
        
        # Multiple related directories
        ("feat", ["src/utils/helper.py", "src/utils/parser.py"], "+def parse():", "feat(src-utils): add parse function"),
        
        # Template without description
        ("feat", ["scripts/hook.py"], None, "feat(scripts): "),  # No description when diff_content is None
    ])
    def test_message_generation(self, commit_type, staged_files, diff_content, expected):
...
class TestAnalyzeGitChangesPatterns:
    ...
    @pytest.mark.parametrize("diff_content,staged_files,expected_type", [
...
class TestHookIntegration:
    ...
    def test_hook_script_execution(self, tmp_path):
        ...
        hook_path = Path(__file__).parent.parent / "scripts" / "conventional_commit_hook.py"
