...
complete -c git-absorb -s m -l message -d 'Commit message body that is given to all fixup commits' -r
complete -c git-absorb -s n -l dry-run -d 'Don\'t make any actual changes'
complete -c git-absorb -l force-author -d 'Generate fixups to commits not made by you'
complete -c git-absorb -l force-detach -d 'Generate fixups even when on a non-branch (detached) HEAD'
complete -c git-absorb -s f -l force -d 'Skip all safety checks as if all --force-* flags were given'
