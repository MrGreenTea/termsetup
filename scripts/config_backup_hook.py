...
@click.command()
@click.option('--max-backups', default=10, help='Maximum number of backups to keep per file')
@click.option('--backup-dir', default=None, help='Custom backup directory (default: ~/.claude/backups)')
def main(max_backups, backup_dir):
    ...
    try:
        ...
        try:
            ...
            print(f"Failed to create backup directory {backup_dir}: {e}", file=sys.stderr)
            return 2
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(f"Failed to create backup of {file_path}: {result.stderr}", file=sys.stderr)
                return 2
