    ...
    try:
        ...
        try:
            ...
            if result.returncode == 0:
                if check_only:
                    print(f"Lua file {file_path} is properly formatted", file=sys.stderr)
                else:
                    print(f"Successfully formatted Lua file: {file_path}", file=sys.stderr)
                return 0
            else:
                print(f"stylua format failed for {file_path}: {result.stderr}", file=sys.stderr)
                return 2
